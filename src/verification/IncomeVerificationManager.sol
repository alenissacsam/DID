...
import "../interfaces/IAadhaarVerificationManager.sol";
...
contract IncomeVerificationManager is AccessControl, ReentrancyGuard {
    ...
    function completeIncomeVerification(
        ...
        _finalizeIncomeVerification(user, success);
        _afterVerificationLog(user, success);
    }

    // Optional path: complete verification using a ZK proof of income >= threshold
    function completeIncomeVerificationWithProof(
        address user,
        bytes memory oracleSignature,
        ...
        // Oracle still signs participation to bind off-chain session/context
        require(
            _verifyOracleSignature(user, true, oracleSignature),
            "Invalid oracle signature"
        );
        ...
        _finalizeIncomeVerification(user, true);
        _afterVerificationLog(user, true);
    }

    function _finalizeIncomeVerification(address user, bool success) internal {
        ...
        if (success) {
            ...
            userRegistry.updateVerificationStatus(
                user,
                IUserIdentityRegistry.VerificationKind.Income,
                true
            );
